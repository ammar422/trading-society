    ...
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" method="POST" action="{{ route('course.update', $course->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    <div class="form-group">
                        <label for="exampleInputName1">Tilte</label>
                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror"
                            id="exampleInputName1" placeholder="Course Tilte" value="{{ $course->title }}">
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="total_hours"
                            ...
                            placeholder="Course total Duration" value="{{ $course->total_hours }}">
                        @error('total_hours')
                            <span class="invalid-feedback" role="alert">
                                <strong>
                                    {{ $message }}
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" name="description" cols="30"
                            rows="10" placeholder="Course Description" value="{{ $course->description }}">{{ $course->description }}</textarea>
                        @error('description')
                            <span class="invalid-feedback" role="alert">
                                <strong>
                                    {{ $message }}
                        ...
                        <select class="form-control @error('category_id') is-invalid @enderror" id="exampleSelectGender"
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    @if ($category->id == $course->category->id) @selected(true) @endif>
